...
def create_course():
    ...
    if form.validate_on_submit():
        form.create_course()
        flash('course create successful', 'success')
...
def edit_course(course_id):
    ...
    if form.validate_on_submit():
        ...
        flash('course updated successful', 'success')
        return redirect(url_for('admin.courses'))
...
def delete_course(course_id):
    ...
    db.session.commit()
    flash('course deleted successful', 'success')
    return redirect(url_for('admin.courses'))
...
def edit_user(user_id):
    ...
    if form.is_submitted():
        ...
        try:
            ...
            flash('user information update successful', 'success')
            return redirect(url_for('admin.users'))
...
def delete_user(user_id):
    if current_user.id == user_id:
        flash("user can not delete by itself", "error")
        return redirect(url_for('admin.users'))
    user = User.query.get_or_404(user_id)
...
def create_live():
    ...
    if form.validate_on_submit():
        form.create_live()
        flash('live create successful', 'success')
...
def send_message():
    ...
    if form.validate_on_submit():
        ...
        flash('system message send successful', 'success')
